...
with st.sidebar:
    ...
    if "API_KEY" not in st.session_state or "API_KEY" in st.session_state:
...
for percent_complete, uploaded_file in enumerate(uploaded_files, start=1):
    ...
    id = hashlib.sha256(image_bytes + st.session_state.session_id.encode()).hexdigest()
    ...
    try:
        if st.session_state.LLM_provider == "OpenAI" and st.session_state.API_KEY:
        ...
        st.error(f"Error occurred!! Check your API key and LLM model selection. Error details: {str(e)}")
        if os.path.exists(f"encrypted_images/{id}.enc"):
            os.remove(f"encrypted_images/{id}.enc")
        if os.path.exists(f"dbs/{st.session_state.session_id}"):
            os.remove(f"dbs/{st.session_state.session_id}")
        st.rerun()
